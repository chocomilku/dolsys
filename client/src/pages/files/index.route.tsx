...
import { DownloadQRModal } from "../../components/modal/DownloadQRModal";
...
export const FilesIndexPage = (): JSX.Element => {
  ...
  const downloadModalOnOpen = (cell_id: number) => {
    setSelectedFileDownload(filesList[cell_id]);
    onDownloadModalOpen();
  };

  const downloadModalOnClose = () => {
          ...
          return (
            <HStack>
              <IconButton
                ...
                colorScheme="green"
                icon={<HiDownload />}
                onClick={() => downloadModalOnOpen(props.row.index)}
              />
              <IconButton
